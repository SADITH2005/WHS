  ...
  <q-page class="q-pa-md bg-gradient-to-br from-gray-50 to-gray-200 min-h-screen">
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div v-if="activeOrder" class="space-y-6">
        ...
        <q-card class="shadow-2xl rounded-2xl border border-black bg-white">
          <q-card-section class="q-pa-md space-y-4">
            <div class="flex justify-between items-center">
              <div class="text-h6 font-black text-black">Verify Items for Packing</div>
              <q-btn-toggle
                v-model="packMode"
                toggle-color="black"
                color="white"
                text-color="black"
                class="border-2 border-black font-bold"
                :options="[
        ...
        <q-card class="shadow-2xl rounded-2xl overflow-hidden border-2 border-black bg-white">
          <q-card-section class="bg-black text-white q-pa-sm flex flex-col sm:flex-row justify-between items-center gap-2">
            <div class="font-bold text-lg">Invoice: {{ activeOrder.invoiceId }} | Branch: {{ activeOrder.branchCode }} - {{ activeOrder.branchName }}</div>
            <q-btn v-if="allPacked" color="positive" label="Mark Order as Packed" @click="finishOrder" class="font-bold text-black bg-green-400" size="sm" />
          </q-card-section>
          
          <q-list separator>
            <q-item v-for="(item, idx) in activeOrder.items" :key="idx" 
                    :class="item.packStatus === 'Packed' ? 'bg-green-100' : (item.packStatus === 'Missing' ? 'bg-red-100 opacity-70' : 'bg-white')">
              <q-item-section>
                <q-item-label class="font-black text-black text-lg" :class="item.packStatus === 'Missing' ? 'line-through' : ''">{{ item.name }}</q-item-label>
                <q-item-label class="font-bold text-black text-md">Barcode: {{ item.barcode }} | Qty: {{ item.qtyInEA }} EA</q-item-label>
                <q-item-label class="mt-1 font-black text-black bg-yellow-300 px-2 py-1 rounded border border-black w-max text-sm">
                  Net Wgt: {{ calculateNetItemWeight(item).toFixed(3) }} kg
                </q-item-label>
              </q-item-section>
              <q-item-section side class="flex items-center flex-row gap-2">
                <q-btn v-if="item.packStatus === 'Pending'" outline color="black" size="sm" icon="block" label="Null" @click="markAsMissing(item)" class="font-bold" />
                <q-icon v-if="item.packStatus === 'Packed'" name="check_circle" color="positive" size="lg" />
                <q-icon v-if="item.packStatus === 'Missing'" name="cancel" color="negative" size="lg" />
                <q-icon v-if="item.packStatus === 'Pending'" name="pending" color="warning" size="lg" />
              </q-item-section>
            </q-item>
          </q-list>
        </q-card>
...
onMounted(async () => {
  ...
  if (bData) {
    branches.value = bData.map(b => ({ label: `${b.code} - ${b.name}`, value: b.code }))
  }
...
const calculateNetItemWeight = (item) => {
  ...
  if (invItem) {
    const rawWeight = parseFloat(invItem.eaWeight) || 0
    if (invItem.eaWeightUnit === 'Grams') {
      eaWeightInKg = rawWeight / 1000
    } else {
      eaWeightInKg = rawWeight
    }
  }
  
  return eaWeightInKg * item.qtyInEA
}
...
const verifySingleWeight = () => {
  ...
  // Expected = Net Weight only (NO box weight)
  const expectedWeight = calculateNetItemWeight(itemInOrder)
  const tolerance = 0.1
  const diff = Math.abs(expectedWeight - scan.value.weight)

  if (diff <= tolerance) {
    verificationResult.value = { message: 'WEIGHT MATCH - OK TO PACK', class: 'bg-green-300 text-black border-black font-black', icon: 'check_circle' }
    itemInOrder.packStatus = 'Packed'
  } else {
    verificationResult.value = { message: `WEIGHT MISMATCH! Expected: ${expectedWeight.toFixed(3)}kg (diff: ${diff.toFixed(3)}kg)`, class: 'bg-red-400 text-black border-black animate-pulse font-black', icon: 'warning' }
  }
  scan.value = { barcode: '', weight: null }
}
...
const verifyMultipleWeight = () => {
  ...
  // Expected = Sum of Net Weights (NO box weight)
  const expectedWeight = expectedMultipleNetWeight.value
  const tolerance = 0.1
  const diff = Math.abs(expectedWeight - scan.value.weight)

  if (diff <= tolerance) {
    verificationResult.value = { message: 'BOX WEIGHT MATCH - OK TO PACK ALL', class: 'bg-green-300 text-black border-black font-black', icon: 'check_circle' }
    // Mark all as packed
    ...
    verificationResult.value = { message: `BOX WEIGHT MISMATCH! Expected: ${expectedWeight.toFixed(3)}kg (diff: ${diff.toFixed(3)}kg)`, class: 'bg-red-400 text-black border-black animate-pulse font-black', icon: 'warning' }
  }
